...
5. **Keep formatting churn out of feature commits**: `standardrb --fix` can touch files unrelated to your change. Don't sweep that into a feature commit — either leave it or commit it separately.
...
1. Run `bundle exec rake` (rspec + standardrb) — this is exactly what CI runs on every push and PR. Fix anything it reports before committing.
